...
class Recommender:
    ...
    def _get_user_anime_list(
        self, user_name: str, batch_size: int = 1000
    ) -> pd.DataFrame:
        """Get the anime list of a user from MyAnimeList.

        Args:
            user_name (str): The name of the user to get the anime list from.
            batch_size (int): The number of anime to fetch in one request. Defaults to 1000.

        Returns:
            pd.DataFrame: A dataframe containing the user's anime list.

        Raises:
            exceptions.InvalidTokenError: If the API token is invalid.
            exceptions.UserNotFoundError: If the user is not found.
            exceptions.RateLimitExceededError: If the API rate limit was exceeded.
            NotImplementedError: For all other not successful HTTP status codes.
        """
        # S
        ...
        # Fetch while there are more animes to fetch
        offset = 0
    ...
    def _do_hybrid_search(
        self, hybrid_query_vec: dict, limit: int, exclude_ids: list
    ) -> SearchResult:
        ...
        res = self.anime_collection.hybrid_search(reqs=reqs, rerank=rerank, limit=limit)

        return res

    def _get_db_entries_by_id(self, ids: list) -> SearchResult:
        res = self.anime_collection.query(expr=f"id in {ids}", output_fields=["*"])
        return res

    def _get_topk_recommendations(
        self, user_anime_list: pd.DataFrame, limit: int
    ) -> dict[int, tuple[int, list]]:
        # Filter out animes that are not in the dataset and drop duplicate
        # animes
        relevant_animes = user_anime_list.loc[
            user_anime_list["id"].isin(self.anime_df["id"]), ["id", "score", "status"]
        ]
        ...
        relevant_animes = relevant_animes[relevant_animes["status"] == "completed"]
        ...
        excluded_ids = (
            user_anime_list.loc[user_anime_list["status"].isin(excluded_status)]
        ...
        for anime in present_animes:
            anime_id = anime["id"]
            anime_score = relevant_animes[relevant_animes["id"] == anime_id][
                "score"
            ].values[0]
    ...
    def _scale_recommendations(
        self,
        recommendations: dict[int, tuple[int, list]],
        default_scaling_factor: float = 7.0,
    ) -> dict[int, list]:
        ...
        for content_id, (user_score, recommendation_list) in recommendations.items():
            scaled_list = []
    ...
    def _flatsort_recommendations(self, recommendations: dict[int, list]) -> list[dict]:
        ...
        return sorted(
            flattend_recommendations, key=lambda x: x["scaled_distance"], reverse=True
        )
    ...
    def recommend_by_username(self, user_name: str, limit: int) -> pd.DataFrame:
        ...
        recommendations = self._get_topk_recommendations(user_anime_list, limit)

        recommendations = self._scale_recommendations(
            recommendations, default_scaling_factor=median_score
        )

        recommendations = pd.DataFrame(self._flatsort_recommendations(recommendations))
        ...
        recommendations.drop(columns=["distance", "user_score", "factor"], inplace=True)

        recommendations.rename(columns={"scaled_distance": "distance"}, inplace=True)
    ...
    def recommend_by_id(self, anime_id: int, limit: int) -> pd.DataFrame:
        ...
        # Perform hybrid search
        query_res = self._do_hybrid_search(vector, limit=limit, exclude_ids=[anime_id])
