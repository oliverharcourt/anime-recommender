...
class AnimeRecommender:
    ...
    def _replace_placeholders(self, data) -> dict:
        ...
        if isinstance(data, dict):
            return {key: self._replace_placeholders(value) for key, value in data.items()}
    ...
    def run(self, autoselect: bool = False) -> pd.DataFrame:
        ...
        config = self._load_config(config_path=self.config_path)
        ...
        collection = self._load_collection(config=config['vector_database'])
...
if __name__ == "__main__":
    ...
    if args.rebuild:
        print(AnimeRecommender(config_path="config.json", rebuild=True).run())

    if args.username:
        print(AnimeRecommender(config_path="config.json", search_str=args.username,
              anime_mode=False, rebuild=False, limit=args.limit).run())
    elif args.anime:
        print(AnimeRecommender(config_path="config.json", search_str=args.anime,
              anime_mode=True, rebuild=False, limit=args.limit).run())
