...
def _create_dataframe(anime_details) -> pd.DataFrame:
    ...
    try:
        anime_df['synopsis'] = anime_df['synopsis'].apply(
...
class DataCollector:
    ...
    def _save_state(self):
        ...
        anime_df = _create_dataframe(self.anime_details)

        output_path = os.path.join(self.data_dir, 'raw', 'anime_data.csv')
        anime_df.to_csv(output_path, mode='a',
    ...
    def _fetch_top_anime(self, limit: int = 500):
        ...
        response = self.session.get(anime_list_url, headers=self.headers, params={
                                    'limit': limit, 'ranking_type': 'all'})
    ...
    def _collect_data(self):
        ...
        while self.download_queue and iterations < self.max_iterations:
            ...
            try:
                related_anime_ids = self._get_related_anime_ids(
                    fetched_details)
            except KeyError as e:
                self.download_queue.append(id)
                print(f"KeyError: {str(e)}")
        ...
        print('Length of seen:', len(self.seen),
              'Length of download queue:', len(self.download_queue))
        if self.debug:
            print('Next anime to fetch:',
                  self.download_queue[0] if self.download_queue else None)
    ...
    def collect(self, anime_ids: list, return_new_ids: bool = False) -> list[dict] | tuple[list[dict], list[int]]:
        ...
        :param anime_ids: List of anime ids
        :param return_new_ids: (optional) Whether to return new anime ids, defaults to False
        :return: List of anime details or a tuple with a list of anime details and a list of new anime ids
        :rtype: list[dict] | tuple[list[dict], list[int]]
        """
        ...
        for anime_id in anime_ids:
            ...
            if fetched_details:
                ...
                related_anime_ids = set(
                    self._get_related_anime_ids(fetched_details))
                new_ids.update(related_anime_ids - self.seen)
            time.sleep(self.request_delay)
